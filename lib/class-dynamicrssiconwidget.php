<?php
/**
 * Dynamic RSS Icon Widget
 *
 * @package PJ_RSS_Icon_Widget
 */

/**
 * DynamicRSSIconWidget extends the WP_Widget class.
 */
class DynamicRSSIconWidget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			false,
			__( 'Dynamic RSS Icon Widget', 'rssiw' ),
			[
	...
	public function widget( $args = [], $instance = [] ) {
		...
		// TODO: Find a way to have a preview if we're in the widget block editor.
		...
		// URL.
		$url = trailingslashit( get_bloginfo( 'rss2_url' ) );
		...
		printf(
			...
			// Styles.
			printf(
				'style="color: %1$s; padding: %2$spx 0px %2$spx %3$spx; background: url(\'%4$sicons/feed-icon-%5$sx%5$s.png\') no-repeat 0 50%%;">',
				esc_attr( $instance['link_color'] ), // color.
				esc_attr( $instance['image_size'] / 2 ), // padding top and bottom.
				esc_attr( $instance['image_size'] + 5 ), // padding left.
				esc_attr( RSSIW_URL ), // URL.
				intval( $instance['image_size'] ) // image size.
			);
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'new_window' ) ); ?>">
					<input
						...
						type="checkbox"
						value="1"
						<?php checked( 1, $settings['new_window'] ); ?> />
